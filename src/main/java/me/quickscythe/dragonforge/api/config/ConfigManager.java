...
public class ConfigManager {
     ...
     */
    public static <T extends Config> T registerConfig(DragonForgeIntegration integration, Class<T> template) {
        try {
            ...
            if (template.isAnnotationPresent(ConfigTemplate.class)) {
                ...
                T content = template.getConstructor(File.class, String.class, DragonForgeIntegration.class).newInstance(file, cf.name(), integration);
                ...
                if (writtenData.has(configField.getName())) {
                    ...
                    if(writtenValue instanceof JSONObject json){
                        if(NestedConfig.class.isAssignableFrom(configField.getType())){
                            NestedConfig nestedConfig = (NestedConfig) configField.getType().getConstructor(Config.class, String.class, DragonForgeIntegration.class).newInstance(content, configField.getName(), content.integration());
     ...
     * @return The config file
     */
    public static <T extends Config> T getConfig(DragonForgeIntegration integration, Class<T> clazz) {
    ...
    public static <E extends Config, D extends NestedConfig<E>> D getNestedConfig(E parent, Class<D> nestedTemplate, String name) {
        ...
        try {
            D nestedConfig = nestedTemplate.getConstructor(Config.class, String.class, DragonForgeIntegration.class).newInstance(parent, name, parent.integration());
